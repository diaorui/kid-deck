...
class AudioPlayerPlugin(Plugin):
    ...
    def ui_js(self) -> str:
        return (
            "let apState = {};\n"
            "function apEsc(s) { return (''+s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\"/g,'&quot;'); }\n"
            "var apTrackFp = '', apTrackPrevIdx = -1;\n"
...
"}\n"
            ...
            "      var fp = s.current_series_files.join(',') + '|' + (s.state === 'playing' ? '1' : '0');\n"
            "      if (fp !== apTrackFp || currentIdx !== apTrackPrevIdx) {\n"
            "        if (fp === apTrackFp) {\n"
            "          var oldRow = apTrackPrevIdx >= 0 && apTrackPrevIdx < trackList.children.length ? trackList.children[apTrackPrevIdx] : null;\n"
            "          if (oldRow) {\n"
            "            oldRow.className = 'track-item upcoming';\n"
            "            oldRow.querySelector('.track-indicator').innerHTML = '\\u25CB';\n"
            "            var oldTitleEl = oldRow.querySelector('.track-title');\n"
            "            if (oldTitleEl && apTrackPrevIdx >= 0 && s.current_series_files[apTrackPrevIdx]) oldTitleEl.innerHTML = apEsc(s.current_series_files[apTrackPrevIdx]);\n"
            "          }\n"
            "          var newRow = currentIdx >= 0 && currentIdx < trackList.children.length ? trackList.children[currentIdx] : null;\n"
            "          if (newRow) {\n"
            "            newRow.className = 'track-item current';\n"
            "            newRow.querySelector('.track-indicator').innerHTML = '\\u25B6';\n"
            "            var newTitleEl = newRow.querySelector('.track-title');\n"
            "            if (newTitleEl && currentIdx >= 0 && s.current_series_files[currentIdx]) {\n"
            "              var ft = s.current_series_files[currentIdx], ef = apEsc(ft);\n"
            "              newTitleEl.innerHTML = '<span class=\"scroll-wrap\" style=\"--dur:' + Math.max(6, ft.length / 6) + 's\"><span>' + ef + '&nbsp;&nbsp;&nbsp;</span><span aria-hidden=\"true\">' + ef + '&nbsp;&nbsp;&nbsp;</span></span>';\n"
            "            }\n"
            "          }\n"
            "        } else {\n"
            "          apTrackFp = fp;\n"
            ...
            "              ? '<span class=\"scroll-wrap\" style=\"--dur:' + Math.max(6, f.length / 6) + 's\"><span>' + escaped + '&nbsp;&nbsp;&nbsp;</span><span aria-hidden=\"true\">' + escaped + '&nbsp;&nbsp;&nbsp;</span></span>'\n"
            "              : escaped;\n"
            "            return '<div class=\"' + cls + '\"><span class=\"track-indicator\">' + indicator + '</span><span class=\"track-title\">' + titleHtml + '</span></div>';\n"
            "          }).join('');\n"
            "        }\n"
            "        apTrackPrevIdx = currentIdx;\n"
            "      }\n"
